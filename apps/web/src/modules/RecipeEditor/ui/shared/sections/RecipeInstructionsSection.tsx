...
export function RecipeInstructionsSection({ form, disabled }: RecipeInstructionsSectionProps) {
  const { t } = useLingui();
  type InstructionItem = RecipeEditorFormValues["instructions"][number];
      ...
      {(field) => (
        ...
        >
          <div className="overflow-hidden rounded-lg border border-border/60 bg-card/60">
            {field.state.value.map((instruction: InstructionItem, index: number) => (
              <div
              ...
              >
                ...
                <div>
                  <Textarea
                    ...
                    onChange={(event) =>
                      field.handleChange(
                        field.state.value.map((item: InstructionItem, currentIndex: number) =>
                ...
                <Button
                  ...
                  onClick={() =>
                    field.handleChange(
                      field.state.value
                        .filter((_: InstructionItem, currentIndex: number) => currentIndex !== index)
                        .map((item: InstructionItem, order: number) => ({ ...item, order })),
                    )
                  }
                >
