...
export function RecipeIngredientsSection({ form, disabled }: RecipeIngredientsSectionProps) {
  const { t } = useLingui();
  type IngredientItem = RecipeEditorFormValues["ingredients"][number];
      ...
      {(field) => (
        ...
        >
          <div className="overflow-hidden rounded-lg border border-border/60 bg-card/60">
            ...
            {field.state.value.map((ingredient: IngredientItem, index: number) => (
              <div
              ...
              >
                <Input
                  ...
                  onChange={(event) =>
                    field.handleChange(
                      field.state.value.map((item: IngredientItem, currentIndex: number) => {
                ...
                <Button
                  ...
                  onClick={() =>
                    field.handleChange(
                      field.state.value
                        .filter((_: IngredientItem, currentIndex: number) => currentIndex !== index)
                        .map((item: IngredientItem, order: number) => ({ ...item, order })),
                    )
                  }
                >
